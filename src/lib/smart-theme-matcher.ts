...
const COLOR_RULES: Array<{ family: ThemeColorFamily; label: string; re: RegExp }> = [
  ...
  { family: 'black', label: '深色系', re: /黑色|黑金|深色|暗色|夜间|暗夜|高对比/ },
  { family: 'purplePink', label: '紫粉色系', re: /紫色|紫粉|玫红|玫瑰红|粉色|少女粉|豆沙粉|樱花粉|珊瑚红|珊瑚粉|科技紫/ },
];
...
const INTENT_RULES: IntentRule[] = [
  ...
  { signal: '商务汇报', re: /工作汇报|项目汇报|经营汇报|商业计划|企业介绍|公司介绍|管理汇报/, scenes: ['商务汇报'], tags: ['商务', '稳重'], themes: ['dune', 'gold-leaf', 'petrol', 'consultant'] },
  ...
  { signal: '教育培训', re: /课程|课件|教学|培训|学校|校园|中学|大学|公开课|答辩/, scenes: ['教育培训', '培训课件'], tags: ['教育', '清新'], themes: ['cornflower', 'vanilla', 'pistachio', 'zephyr'] },
  { signal: '旅游文旅', re: /旅行|旅游|攻略|行程|景点|城市介绍|文旅|酒店|民宿|度假|游学/, scenes: ['旅游出行', '文旅介绍'], tags: ['自然', '生活'], themes: ['finesse', 'elysia', 'oasis', 'cornfield'] },
  { signal: '餐饮咖啡', re: /咖啡|餐饮|美食|菜品|烘焙|甜品|饮品|奶茶|餐厅|茶饮/, scenes: ['餐饮美食', '生活方式'], tags: ['温暖', '生活'], themes: ['finesse', 'clementa', 'wine', 'chocolate'] },
  { signal: '品牌营销', re: /品牌|营销|推广|广告|新品|发布会|活动策划|传播|社交媒体/, scenes: ['营销提案', '品牌介绍'], tags: ['创意', '品牌'], themes: ['gamma', 'atmosphere', 'dialogue', 'gold-leaf'] },
  { signal: '美妆时尚', re: /美妆|护肤|彩妆|时尚|服装|珠宝|香氛|女性品牌/, scenes: ['美妆时尚'], tags: ['雅致', '柔和'], themes: ['ashrose', 'coral-glow', 'creme', 'wine'] },
  { signal: '婚礼情感', re: /婚礼|婚庆|求婚|爱情|恋爱|纪念日|周年|浪漫|相守/, scenes: ['婚礼庆典', '情感分享'], tags: ['情感', '浪漫'], themes: ['coral-glow', 'ashrose', 'peach', 'wine'] },
  { signal: '医疗健康', re: /医疗|健康|医院|护理|药品|医学|康复|心理|营养/, scenes: ['医疗健康'], tags: ['清新', '可靠'], themes: ['seafoam', 'sage', 'tranquil', 'vanilla'] },
  { signal: '自然环保', re: /环保|生态|可持续|绿色发展|植物|农业|乡村振兴|碳中和/, scenes: ['环保公益', '乡村文旅'], tags: ['自然', '清新'], themes: ['sprout', 'plant-shop', 'sage', 'cornfield'] },
  { signal: '传统文化', re: /国风|古风|中式|传统文化|非遗|历史|博物馆|古镇|古村|文化遗产/, scenes: ['文化教育', '品牌故事'], tags: ['传统', '人文'], themes: ['terracotta', 'kraft', 'cornfield', 'wine'] },
  { signal: '高端品牌', re: /高端|轻奢|奢华|精品|豪华|尊贵|典藏/, scenes: ['高端品牌'], tags: ['高端', '质感'], themes: ['gold-leaf', 'dune', 'chocolate', 'aurum'] },
  { signal: '生活方式', re: /生活方式|家居|日常|社区|成长|个人分享|兴趣/, scenes: ['生活方式'], tags: ['生活', '温暖'], themes: ['finesse', 'creme', 'plant-shop', 'breeze'] },
];
...
export function detectThemeIntent(text: string, scene?: string, tone?: Tone): ThemeIntent {
  ...
  if (/白色简约|白色极简|纯白简约|简约白|极简白|白色风格/.test(source)) preferredThemeIds.unshift('howlite');
  if (/红色主题|红色系|中国红|国旗红|喜庆红|大红|正红|朱红|酒红|红金|节庆红/.test(source)) {
    preferredThemeIds.unshift('dialogue', 'wine');
  ...
  if (/玫红|玫瑰红|粉色|少女粉|豆沙粉|樱花粉/.test(source)) {
    preferredThemeIds.unshift('ashrose');
    explicitColorLabel = '玫瑰粉';
  }
  if (/深蓝|海军蓝|藏蓝/.test(source)) preferredThemeIds.unshift('blues', 'marine');
  if (/天蓝|校园|学校|中学|教育|培训/.test(source) && /蓝/.test(source)) preferredThemeIds.unshift('cornflower');
  if (/咖啡|田园|文旅|乡村|自然|优雅米绿|米绿/.test(source)) preferredThemeIds.unshift('finesse');
  if (/红色主题|红色系|中国红|国旗红|喜庆红|大红|正红|朱红|酒红|红金|节庆红/.test(source)) {
    preferredThemeIds.unshift('dialogue');
  }
  if (colorMatches.some(match => match.family === 'blue')) {
    if (/教育|培训|校园|学校|中学|大学/.test(source)) preferredThemeIds.unshift('cornflower');
    else if (/科技|AI|人工智能|数字化|软件|互联网/.test(source)) preferredThemeIds.unshift('blue-steel');
    else if (!/深蓝|海军蓝|藏蓝/.test(source)) preferredThemeIds.unshift('consultant');
  }
...
function scoreTheme(theme: ThemeData, intent: ThemeIntent): number {
  ...
  const prefersWhiteTheme = intent.colorFamilies.includes('whiteGray')
  ...
  if (theme.colorFamily === 'whiteGray' && !prefersWhiteTheme) score -= 55;
  if (theme.id === 'howlite' && !prefersWhiteTheme) score -= 75;

  // 蓝湾智策只在用户明确选择蓝色或内容画像把它列为候选时出现，
  // 避免“专业”这一单一信号把所有省心模式都吸到默认商务蓝。
  if (theme.id === 'consultant' && !intent.colorFamilies.includes('blue') && preferenceRank < 0) score -= 38;
