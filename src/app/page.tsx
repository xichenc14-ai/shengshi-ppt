...
function SubscribeHandler({ onSubscribe }: { onSubscribe: (planId: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (plan && ['shengxin', 'supreme', 'basic', 'pro', 'vip'].includes(plan)) {
      onSubscribe(plan);
      // 清除URL参数
      router.replace('/');
    }
...
export default function Home() {
  ...
  const generateDirect = useCallback(async () => {
    ...
    const imgCreditsPerImg = directImgMode === 'ai' || directImgMode === 'ai-pro' ? IMG_CREDIT_PER_PAGE : 0;
    const estImgCount = Math.ceil(effectivePages / 2);
    const creditsToDeduct = effectivePages * CREDIT_PER_PAGE + estImgCount * imgCreditsPerImg;
  ...
  const handleGeneratePPT = useCallback(() => {
    ...
    // 🚨 v10.7.1 fix: 专业模式也走 outline 流程，不再跳过大纲编辑页面
    setPhase('streaming');
    generateOutline();
  ...
  const handleSubscribe = useCallback((planId: string) => {
    ...
    const planMap: Record<string, { id: string; name: string; price: string }> = {
      ...
      'supreme': { id: 'supreme', name: '🏆 尊享会员', price: '¥39.9/月' },
      'basic': { id: 'shengxin', name: '✨ 省心会员', price: '¥19.9/月' },
      'pro': { id: 'supreme', name: '🏆 尊享会员', price: '¥39.9/月' },
      'vip': { id: 'supreme', name: '🏆 尊享会员', price: '¥39.9/月' },
    };
            ...
            <div className="relative max-w-3xl mx-auto px-3 md:px-6">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4 md:hidden">
                <button
                  ...
                  className="group p-4 bg-white rounded-xl border-2 border-gray-100 hover:border-purple-200 transition-all text-left"
                >
                ...
                <button
                  ...
                  className="group p-4 bg-white rounded-xl border-2 border-gray-100 hover:border-amber-200 transition-all text-left"
                >
                  <div className="flex items-center gap-2 mb-2">
          ...
          <div className="max-w-3xl mx-auto px-4 md:px-6 pt-4 pb-24">
            ...
            {phase === 'input' && (
              <>
                ...
                {/* Input card */}
                <div className="relative bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6">
                  {/* Textarea - full width */}
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute bottom-2.5 left-2.5 w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-[#5B4FE9] hover:bg-[#F5F3FF] transition-all"
                      title="上传附件"
                    >
                  ...
                  {mode === 'direct' && (
                    <div className="mt-4 pt-4 border-t border-gray-100 space-y-5 max-h-[55vh] overflow-y-auto -mx-1 px-1">
                      {/* 主题风格 */}
                      <div>
                        <h3 className="text-sm font-bold text-gray-800 mb-3">主题风格</h3>
                  ...
                  <button
                    ...
                    disabled={!hasInput}
                    className={`w-full mt-5 py-4 rounded-xl text-sm font-bold transition-all ${
                      hasInput
                        ? 'bg-gradient-to-r from-[#5B4FE9] to-[#8B5CF6] text-white shadow-lg shadow-purple-300/30 hover:shadow-xl hover:shadow-purple-400/40 active:scale-[0.98]'
                  ...
                  {!loading && (
                    <div className="mt-4">
                      <button
                        ...
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-[#5B4FE9] to-[#8B5CF6] text-white text-sm font-semibold shadow-md shadow-purple-200/50 hover:shadow-lg hover:shadow-purple-300/50 active:scale-[0.98] transition-all"
                      >
                        🪄 确认生成 PPT
                      </button>
                    </div>
