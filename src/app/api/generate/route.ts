...
async function atomicDeductCredits(sb: NonNullable<ReturnType<typeof getSupabase>>, userId: string, amount: number, description: string): Promise<{ success: boolean; balance?: number; error?: string }> {
  try {
    ...
    if (!rpcErr) {
      return { success: true, balance: (rpcResult as DeductCreditsRpcResult | null)?.new_balance ?? undefined };
    }
  } catch {}
...
- 总共生成${slideCount}页幻灯片`;
    ...
    const presentation: Presentation = {
      ...
      theme: topic,
      templateId: 'ocean',
      slides: (parsed.slides || []).map((s: ParsedSlideLike, i: number) => ({
        id: generateId(),
        type: s.type || 'content',
