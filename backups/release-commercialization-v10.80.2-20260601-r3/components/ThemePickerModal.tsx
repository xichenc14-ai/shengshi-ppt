...
export default function ThemePickerModal({ open, currentThemeId, currentTone, currentImgSrc, onThemeChange, onToneChange, onImgChange, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-[420px] max-w-[96vw] max-h-[88vh] overflow-y-auto animate-modal-in" onClick={(e) => e.stopPropagation()}>
        {/* 顶部渐变装饰条 */}
        ...
        <div className="p-3.5 sm:p-5">
          ...
          <div className="mb-4 sm:mb-5">
            ...
            <div className="mb-3 rounded-xl border border-indigo-100 bg-indigo-50/60 p-2.5 sm:p-3">
              ...
              <div className="rounded-lg bg-white border border-indigo-100 p-2">
                ...
                <div className="relative h-20 sm:h-24 rounded-lg border border-slate-200 p-2 sm:p-3 overflow-hidden" style={{ backgroundColor: bgColor }}>
                  ...
                  <div className="absolute left-2 bottom-2 sm:left-3 sm:bottom-3">
                    <p className="text-base sm:text-xl font-black leading-none" style={{ color: fontColor }}>省心PPT</p>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-3 gap-1 sm:gap-2">
              {COLOR_THEMES.map(t => (
                <button
                  ...
                  className={`flex items-center gap-1 px-1.5 py-1.5 sm:gap-1.5 sm:px-3 sm:py-2.5 rounded-lg sm:rounded-xl border-2 transition-all ${
                    currentThemeId === t.id
                ...
                >
                  <div className="flex gap-0.5 flex-shrink-0">
                    {t.colors.map((c, i) => (
                      <div key={i} className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 rounded-full border border-gray-200/50" style={{ backgroundColor: c }} />
                    ))}
                  </div>
                  <span className={`text-[10px] sm:text-xs font-medium truncate ${currentThemeId === t.id ? 'text-[#4338CA]' : 'text-gray-600'}`}>{t.name}</span>
                  {currentThemeId === t.id && (
