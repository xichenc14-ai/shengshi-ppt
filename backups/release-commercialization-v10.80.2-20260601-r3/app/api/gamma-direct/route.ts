...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const uploadedFileNames = Array.isArray(uploadedFiles)
      ? uploadedFiles.map((f: UploadedFileLike) => String(f?.name || '').trim()).filter(Boolean)
      : [];
    ...
    console.error('Gamma direct error:', errorMessage);
    console.log(`[GammaDirect] ERROR reason=${errorMessage || 'unknown'} code=INTERNAL_ERROR status=500`);
    return NextResponse.json({ error: errorMessage || '生成失败' }, { status: 500 });
  }
}
