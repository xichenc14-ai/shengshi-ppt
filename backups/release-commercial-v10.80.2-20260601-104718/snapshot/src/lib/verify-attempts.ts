...
type InMemoryAttemptEntry = {
  count: number;
  resetAt: number;
};

type InMemoryAttempts = Record<string, InMemoryAttemptEntry>;
...
export async function checkVerifyAttemptsDB(phone: string): Promise<{
  ...
  if (!sb) {
    ...
    const entry = (global as unknown as { __verifyAttempts?: InMemoryAttempts }).__verifyAttempts?.[key];
...
export async function recordVerifyAttempt(phone: string): Promise<void> {
  ...
  if (!sb) {
    // 内存版
    const globalStore = global as unknown as { __verifyAttempts?: InMemoryAttempts };
    if (!globalStore.__verifyAttempts) globalStore.__verifyAttempts = {};
    const key = `verify_attempts:${phone}`;
    const now = Date.now();
    const entry = globalStore.__verifyAttempts[key];
    if (!entry || now > entry.resetAt) {
      globalStore.__verifyAttempts[key] = { count: 1, resetAt: now + 10 * 60 * 1000 };
    } else {
      entry.count++;
    }
    return;
...
export async function clearVerifyAttempts(phone: string): Promise<void> {
  ...
  if (!sb) {
    const globalStore = global as unknown as { __verifyAttempts?: InMemoryAttempts };
    if (globalStore.__verifyAttempts) delete globalStore.__verifyAttempts[`verify_attempts:${phone}`];
    return;
  }
