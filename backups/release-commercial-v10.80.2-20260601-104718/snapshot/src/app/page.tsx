...
type SmartGammaPayloadState = {
  themeId?: string;
  tone?: string;
  imageOptions?: { source?: string; [key: string]: unknown };
  gammaPayload?: Record<string, unknown>;
  [key: string]: unknown;
};
type OutlineStreamEvent =
  | { type: 'stage'; stage?: string; message?: string }
  | { type: 'slides'; slides?: SlideItem[]; total?: number; current?: number }
  | { type: 'error'; message?: string }
  | { type: 'complete'; data?: OutlineResultState };
...
export default function Home() {
  ...
  const [smartGammaPayload, setSmartGammaPayload] = useState<SmartGammaPayloadState | null>(null); // 省心模式 AI 生成的完整参数
  ...
  const [feedbackVote, setFeedbackVote] = useState<'up' | 'down' | ''>('');
  ...
  const runOutlineRequest = useCallback(async (outlinePayload: OutlinePayload) => {
    const runOnce = async () => {
      ...
      let od: OutlineResultState | null = null;
      try {
        ...
        while (true) {
          ...
          while (newlineIdx >= 0) {
            ...
            let evt: OutlineStreamEvent;
            try {
              evt = JSON.parse(line) as OutlineStreamEvent;
            } catch {
              continue;
            }
            ...
            if (evt.type === 'stage') {
              ...
              const stageKey = typeof evt.stage === 'string' ? evt.stage : '';
              const stageMsg = evt.message || fallbackMsgByStage[stageKey] || '正在处理中...';
        ...
        const fetchErrMessage = fetchErr instanceof Error ? fetchErr.message : '';
        if (fetchErrMessage === 'STREAM_FALLBACK') {
        ...
        throw new Error(fetchErrMessage || '网络错误，请检查网络连接后重试');
      } finally {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const msg = err instanceof Error ? err.message : '大纲生成失败';
  ...
  const handleGeneratePPT = useCallback(() => {
    ...
    setPhase('streaming');
    generateOutline();
  }, [user, hasInput, generateOutline, openLogin]);
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      try {
        ...
        if (consecutiveStatusErrors >= 4) {
          throw new Error(networkErr instanceof Error ? networkErr.message : '渲染状态查询失败');
        }
  ...
  const confirmAndGenerate = useCallback(async () => {
    ...
    try {
      ...
      const gammaRequestBody: Record<string, unknown> = {
        inputText: md,
        textMode: tm,
        auto: mode === 'smart',
        strictPreserve: strictPreserveEnabled,
      ...
      let renderResult: { gd: { generationId: string }; lastStatusData: GammaStatusData; finalExportUrl: string } | null = null;
      ...
      for (let attempt = 1; attempt <= maxRenderAttempts; attempt++) {
        try {
          ...
          const msg = renderErr instanceof Error ? renderErr.message : '渲染失败';
      ...
      if (!isLikelyNavigatingAway()) {
        ...
        if (deductedCredits > 0) {
          rollbackCredits(deductedCredits, e instanceof Error ? e.message : '生成失败');
        }
  ...
  const ensurePptxGenerationId = useCallback(async () => {
    ...
    let created: { generationId?: string } | null = null;
    try {
  ...
  const handleExportPPT = async () => {
    ...
    try {
      ...
      const permissionData = await permissionRes.json().catch(() => ({} as Record<string, unknown>));
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : 'PDF 导出失败，请稍后重试');
    } finally {
  ...
  const handleOneTimeDownload = async () => {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : '单次付费下载失败，请稍后重试');
    } finally {
      setPayingOnce(false);
    }
  };
    ...
    try {
      const res = await fetch('/api/feedback', {
        ...
        body: JSON.stringify({
          ...
          generationId: result.generationId,
          topic: topic.trim(),
          pptTitle: result.title || '省心PPT',
          pageCount: result.actualPages || pageCount,
          imageMode: imgMode,
        }),
      });
      ...
      setFeedbackError(e instanceof Error ? e.message : '反馈提交失败');
    } finally {
      setFeedbackSubmitting(false);
    }
  };
                              ...
                              {(['professional','casual','creative','bold','traditional'] as const).map(t => (
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    setSmartGammaPayload((prev) => prev ? {
                                      ...prev,
                                      tone: t,
                                      gammaPayload: prev.gammaPayload ? {
                                        ...prev.gammaPayload,
                                        tone: t,
                                        textOptions: {
                                          ...(typeof prev.gammaPayload.textOptions === 'object' && prev.gammaPayload.textOptions
                                            ? prev.gammaPayload.textOptions
                                            : {}),
                                          tone: t,
                                        },
                                      } : undefined,
                                  ...
                                  onClick={() => {
                                    ...
                                    setSmartGammaPayload((prev) => prev ? {
                                      ...
                                      gammaPayload: prev.gammaPayload ? {
                                        ...prev.gammaPayload,
                                        imageOptions: {
                                          ...(typeof prev.gammaPayload.imageOptions === 'object' && prev.gammaPayload.imageOptions
                                            ? prev.gammaPayload.imageOptions
                                            : {}),
                                          source: src,
                                        },
                                      } : undefined,
      ...
      {phase === 'result' && result && !loading && (
        <div className="flex-1 bg-gradient-to-b from-purple-50/30 to-white">
          <div className="max-w-[1400px] mx-auto px-3 md:px-6 pt-6 md:pt-8 pb-16">
            ...
            <div className="sx-glass rounded-[24px] border border-indigo-100/70 p-4 md:p-5 mb-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h3 className="text-base font-black text-slate-900">这份 PPT 对你有帮助吗？</h3>
                {feedbackSubmitted && <span className="text-xs text-emerald-600 font-semibold">反馈已提交，感谢支持</span>}
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  onClick={() => setFeedbackVote('up')}
                  className={`px-3 py-2 rounded-xl border text-sm font-semibold transition-all ${feedbackVote === 'up' ? 'bg-emerald-50 border-emerald-300 text-emerald-700' : 'bg-white border-indigo-200 text-slate-700'}`}
                >
                  👍 点赞
                </button>
                <button
                  onClick={() => setFeedbackVote('down')}
                  className={`px-3 py-2 rounded-xl border text-sm font-semibold transition-all ${feedbackVote === 'down' ? 'bg-rose-50 border-rose-300 text-rose-700' : 'bg-white border-indigo-200 text-slate-700'}`}
                >
                  👎 点踩
                </button>
                  ...
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <button
                        ...
                        className={`text-xl leading-none ${feedbackRating >= n ? 'text-amber-400' : 'text-slate-300'}`}
                        aria-label={`评分 ${n} 星`}
                      >
                        ★
                      </button>
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                />
                <span className="text-xs text-slate-500 w-10 text-right">{feedbackRating}分</span>
              </div>
              <textarea
                value={feedbackComment}
                onChange={(e) => setFeedbackComment(e.target.value.slice(0, 600))}
                placeholder="欢迎告诉我们：哪里好用、哪里还需要优化（最多600字）"
                className="mt-3 w-full min-h-[96px] rounded-xl border border-indigo-100 bg-white px-3 py-2 text-sm text-slate-700 outline-none focus:border-indigo-300"
              />
              {feedbackError && <p className="mt-2 text-xs text-rose-500">{feedbackError}</p>}
              <div className="mt-3 flex justify-end">
                <button
                  onClick={handleSubmitFeedback}
                  disabled={feedbackSubmitting || feedbackSubmitted}
                  className="px-4 py-2 rounded-xl sx-primary-btn text-white text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed"
                >
