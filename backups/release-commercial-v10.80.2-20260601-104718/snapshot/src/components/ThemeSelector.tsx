...
const MAX_THEMES_PER_PANEL = 12; // 手机端 3行 × 4列
...
function getThemeCard(theme: ThemeData, active: boolean, onChange: (themeId: string) => void) {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[40px] rounded-md border border-black/5 overflow-hidden p-1 sm:h-[62px] sm:rounded-lg sm:p-2 md:h-[70px] md:p-2.5" style={{ backgroundColor: bg }}>
        <span
        ...
        <div className="absolute left-1 bottom-1 right-1 sm:left-2 sm:bottom-2 sm:right-2">
          <div className="text-[10px] font-black leading-none truncate sm:text-[16px] md:text-[18px]" style={{ color: textColor }}>
            省心PPT
          </div>
          <div className="mt-0.5 flex items-center gap-1 sm:mt-1 sm:gap-1.5">
            <span className="text-[7px] font-bold truncate sm:text-[10px]" style={{ color: textColor }}>{theme.nameZh}</span>
          ...
          >
            <div className="flex gap-0.5 mb-0.5 sm:mb-1">
              {cat.colors.slice(0, 3).map((c, i) => (
                <span key={`${cat.id}-${i}`} className="w-2.5 h-2.5 rounded-full shadow-sm border border-white/60 sm:w-4 sm:h-4" style={{ backgroundColor: c }} />
              ))}
            </div>
            <span className="text-[8px] font-medium text-gray-600 whitespace-nowrap sm:text-[10px]">{cat.name}</span>
          </button>
        ))}
      </div>

      <div className="bg-[#FAFBFE] rounded-xl p-2 sm:p-3 md:p-4 mt-2 animate-fade-in">
        <div className="flex items-center justify-between gap-2 mb-2.5 sm:mb-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-xs sm:text-sm font-semibold text-gray-700 truncate">{selectedMeta.name}</span>
            <span className="text-[10px] sm:text-xs text-gray-400 shrink-0">· {selectedMeta.count} 个主题</span>
          </div>
          {selectedTheme && (
        ...
        <div className="grid grid-cols-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1 sm:gap-2">
