...
type FunnelTooltipEntry = { color?: string; payload?: DataPoint };
type FunnelTooltipProps = { active?: boolean; payload?: FunnelTooltipEntry[]; total: number };

const FunnelTooltipContent: React.FC<FunnelTooltipProps> = ({ active, payload, total }) => {
    ...
    <div style={{ width, height: height + (title ? 30 : 0) }}>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <RechartsFunnelChart>
          {showTooltip && (
            <Tooltip
              content={<FunnelTooltipContent total={totalValue} />}
              {...buildTooltipConfig()}
            />
          )}
          <Funnel
