...
export const THEME_DATABASE: ThemeData[] = [
  ...
  {
    id: 'borealis', name: 'Borealis', nameZh: '霓虹蓝绿',
  ...
  {
    id: 'cigar', name: 'Cigar', nameZh: '高端商务',
...
export const COLOR_CATEGORIES: Array<{ id: string; name: string; emoji: string; count: number; colors: string[] }> = [
  { id: 'blue', name: '蓝色系', emoji: '🟦', count: 0, colors: ['#1E3A5F', '#2563EB', '#3B82F6'] },
  { id: 'gray', name: '黑白灰', emoji: '⬛', count: 0, colors: ['#111827', '#6B7280', '#F3F4F6'] },
  { id: 'purple', name: '紫色系', emoji: '🟪', count: 0, colors: ['#7C3AED', '#A78BFA', '#06B6D4'] },
  { id: 'brown', name: '棕米大地', emoji: '🟫', count: 0, colors: ['#92400E', '#D97706', '#78350F'] },
  { id: 'pink', name: '粉色系', emoji: '🩷', count: 0, colors: ['#FDF2F8', '#EC4899', '#BE185D'] },
  { id: 'warm', name: '暖色活力', emoji: '🟧', count: 0, colors: ['#EA580C', '#22C55E', '#14B8A6'] },
  { id: 'gold', name: '金色奢华', emoji: '🪙', count: 0, colors: ['#111827', '#F59E0B', '#FCD34D'] },
];

export function getThemesByCategory(category: string): ThemeData[] {
  // 使用已校对分类作为展示分组，避免仅靠阈值导致边界主题漂移
...
export function getThemeCategoryByBackground(background: string): string {
  ...
  // yellow / gold
  if (h >= 45 && h < 75) return 'gold';
  // beige / earth / brown
  if (h >= 20 && h < 45) return 'brown';
  // green / teal
  if (h >= 75 && h < 200) return 'warm';
  // red / orange
  if (h < 20 || h >= 345) return 'warm';

  return 'gray';
}

const THEMES_BY_BG_CATEGORY: Record<string, ThemeData[]> = THEME_DATABASE.reduce((acc, theme) => {
...
for (const cat of COLOR_CATEGORIES) {
  const list = THEMES_BY_BG_CATEGORY[cat.id] || [];
