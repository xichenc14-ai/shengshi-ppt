...
export default function ThemePickerModal({ open, currentThemeId, currentTone, currentImgSrc, onThemeChange, onToneChange, onImgChange, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl w-[420px] max-w-[94vw] max-h-[88vh] overflow-y-auto animate-modal-in" onClick={(e) => e.stopPropagation()}>
        {/* 顶部渐变装饰条 */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
              {COLOR_THEMES.map(t => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2 py-2 sm:px-3 sm:py-2.5 rounded-xl border-2 transition-all ${
                    currentThemeId === t.id
                ...
                >
                  <div className="flex gap-0.5 flex-shrink-0">
                    {t.colors.map((c, i) => (
                      <div key={i} className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full border border-gray-200/50" style={{ backgroundColor: c }} />
                    ))}
                  </div>
                  <span className={`text-[11px] sm:text-xs font-medium truncate ${currentThemeId === t.id ? 'text-[#4338CA]' : 'text-gray-600'}`}>{t.name}</span>
                  {currentThemeId === t.id && (
