...
function getThemeCard(theme: ThemeData, active: boolean, onChange: (themeId: string) => void) {
  ...
  return (
    <button
      ...
      className={`relative rounded-xl border-2 px-1 py-1 sm:px-1.5 sm:py-1.5 md:px-2 md:py-2 text-left transition-all ${
        active
    ...
    >
      ...
      <div className="relative h-[54px] rounded-lg border border-black/5 overflow-hidden p-1.5 sm:h-[62px] sm:p-2 md:h-[70px] md:p-2.5" style={{ backgroundColor: bg }}>
        <span
          className="absolute top-1 right-1 h-4 px-1 rounded-md border border-white/40 text-[9px] font-semibold leading-4 shadow-sm sm:top-1.5 sm:right-1.5 sm:h-4.5 sm:px-1.5 sm:text-[10px] sm:leading-[18px]"
          style={{ backgroundColor: accent, color: textColor }}
        >
          强调
        </span>
        <div className="absolute left-1.5 bottom-1.5 right-1.5 sm:left-2 sm:bottom-2 sm:right-2">
          <div className="text-[14px] font-black leading-none truncate sm:text-[16px] md:text-[18px]" style={{ color: textColor }}>
            省心PPT
          </div>
          <div className="mt-0.5 flex items-center gap-1 sm:mt-1 sm:gap-1.5">
            <span className="text-[9px] font-bold truncate sm:text-[10px]" style={{ color: textColor }}>{theme.nameZh}</span>
...
export default function ThemeSelector({ value, onChange }: ThemeSelectorProps) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide sm:gap-2">
        {categoryOptions.map((cat) => (
          <button
            key={cat.id}
            onClick={() => setSelectedCategory(cat.id)}
            className={`flex-shrink-0 flex flex-col items-center p-1.5 rounded-xl border-2 transition-all duration-200 sm:p-2 ${
          ...
          >
            <div className="flex gap-1 mb-1">
              {cat.colors.slice(0, 3).map((c, i) => (
                <span key={`${cat.id}-${i}`} className="w-3.5 h-3.5 rounded-full shadow-sm border border-white/60 sm:w-4 sm:h-4" style={{ backgroundColor: c }} />
              ))}
            </div>
