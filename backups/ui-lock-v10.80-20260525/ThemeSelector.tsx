...
function getThemeCard(theme: ThemeData, active: boolean, onChange: (themeId: string) => void) {
  ...
  return (
    <button
      ...
      className={`relative rounded-xl border-2 px-1.5 py-1.5 md:px-2 md:py-2 text-left transition-all ${
        active
    ...
    >
      ...
      <div
        className="relative h-[62px] rounded-lg border border-black/5 overflow-hidden p-2 sm:h-[66px] md:h-[70px] md:p-2.5"
        style={{ backgroundColor: bg }}
      >
        <span
          className="absolute top-1.5 right-1.5 h-4.5 px-1.5 rounded-md border border-white/40 text-[10px] font-semibold leading-[18px] shadow-sm"
          style={{ backgroundColor: accent, color: textColor }}
        >
          强调
        </span>
        <div className="absolute left-2 bottom-2 right-2">
          <div className="text-[18px] font-black leading-none truncate" style={{ color: textColor }}>
            省心PPT
          </div>
          <div className="mt-1 flex items-center gap-1.5">
            <span className="text-[10px] font-bold truncate" style={{ color: textColor }}>{theme.nameZh}</span>
